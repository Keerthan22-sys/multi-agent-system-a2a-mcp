...
This project wires several **FastMCP** servers together: lightweight “tool” servers (news, weather, FX, images) feed **agents** that coordinate through a tiny file-based mailbox (**post office** under `synapse/protocol/`). A **Streamlit** UI triggers the Scout and Publisher tools to produce an article grounded in aggregated signals.
...
- **scout** — Drives contextualist and media-engine, merges context + media for the Publisher.
- **publisher** — OpenAI Responses API generates the final brief from scout output.
...
| 4 | `python agents/contextualist_agent/main.py` |
| 5 | `python agents/scout_agent/main.py` |
| 6 | `python agents/publisher_agent/main.py` |
...
- **Timeouts or empty context:** Confirm all six MCP processes are listening and `.env` keys are valid for the upstream APIs.

## Project layout

- `agents/` — Contextualist, Scout, Publisher FastMCP entrypoints.
- `mcp-servers/` — Stateless tool MCP servers backed by HTTP APIs.
